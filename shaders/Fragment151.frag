...
vec3 renderingFunc(vec2 uv){
    ...
    float rippleR = sin(length(uv) * 10.0 - time * 4.0 - 0.4);
    float rippleG = sin(length(uv) * 10.0 - time * 4.0);
    float rippleB = sin(length(uv) * 10.0 - time * 4.0 + 0.5);

    color.r += rippleR * voronoiStrength/voronoi(uv + vec2(0.0, 0.0), 8.0).x;
    color.g += rippleG * voronoiStrength/voronoi(uv + vec2(0.0, 0.0), 8.0).x;
    color.b += rippleB * voronoiStrength/voronoi(uv + vec2(0.0, 0.0), 8.0).x;
