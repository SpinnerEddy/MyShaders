...
vec3 renderingFunc(vec2 uv){
    ...
    color += mix(skyTexture(uv, 3.0), vec3(0.8863, 0.8196, 0.8196), orb(uv - vec2(0.5, 0.7), 0.2 + 0.03 * sin(time * 6.0)));
...
void main(){
    ...
    vec2 uv2 = cLog(uv);
    uv2.y += time;
    vec2 uv3 = cDiv(uv2, cExp(vec2(0, angle)) * cos(angle));
    uv3.x = mod(uv3.x - time * 0.7, scale);
    vec2 uv4 = cExp(uv3) * r1;

    vec3 color = vec3(0.0);

    vec2 outputUv = mix(uv, uv4, sin(time) * 0.5 + 0.5);

    color += renderingFunc(uv4);
