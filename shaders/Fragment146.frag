...
vec3 renderingFunc(vec2 uv){
	...
	if(hit){
		...
		float diff = clamp(dot(normal, normalize(lightDir)), 0.0, 1.0);
		color += mix(vec3(0.0, 0.0, 0.0), vec3(1.0, 1.0, 1.0), diff);

		float cd = length(p);
		color *= mix(vec3(1.0), vec3(smoothstep(-cd * 0.04, cd * 0.04, sdGyroid(-lightDir, 10.0))), step(1.03, cd));
		color /= cd * cd;
	}
