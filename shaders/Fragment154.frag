...
vec3 renderingFunc(vec2 uv){
    ...
    vec2 polar = vec2(a, 0.1 / length(uv) + time * 0.1);
    vec2 polar2 = polar * 20.0;
    polar2.x += step(1.0, mod(polar2.y, 2.0)) * 0.5;
    vec2 fPos = (fract(polar2) - 0.5) * 2.0;
    vec2 iPos = floor(polar2);

    // 
    // color += 0.12 / length(fPos) * (sin(iPos.y + time * 5.0) + 1.0) * 0.5;
    color += lattice(polar, 20.0);

    // whiteout center
    color += smoothstep(0.4, 0.1, length(uv));
