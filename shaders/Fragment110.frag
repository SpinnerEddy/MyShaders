...
float rectFlowar(vec2 uv, float tCoef){
    ...
    return smoothstep(0.05, -0.05, sdFrameRect(p, vec2(0.6*sin(length(uv*3.0) - time * 2.0 + tCoef) + 0.5), 0.5 + cos(length(p) * pi) * 0.5));
}

vec3 renderingFunc(vec2 uv){
    vec3 color = COLOR_N;

    color.r += rectFlowar(uv, -0.05);
    color.g += rectFlowar(uv, 0.0);
    color.b += rectFlowar(uv, 0.05);
