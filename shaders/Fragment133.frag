...
vec3 starTextureN(vec2 uv){
    ...
    // col += smoothstep(0.4, -0.4, sdStar(fPos, 0.31));
    col += COLOR_N;
    float rand = random2d(iPos);
...
vec3 renderingFunc(vec2 uv){
    ...
    // color += step(sin(sdStar(uv * 10.0, 0.1) - time * 3.0), 0.0);
    // color += starTextureN(uv);
    // color += starTextureC(uv);
