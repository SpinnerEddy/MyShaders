...
vec3 renderingFunc(vec2 uv){
    ...
    fPos *= rotate(sin(length(fPos) * 2.2));
    float ft1 = fract((time * 1.2 - length(uv) * 0.22) * 0.1);  // 0 ~ 1
    ...
    vec2 offset2 = vec2(mix(-0.9, 0.8, bounceOut(t2) - bounceOut(t4)), mix(-0.9, 0.8, bounceOut(t1) - bounceOut(t3)));
    vec2 offset22 = vec2(mix(0.5, 0.7, bounceOut(t2) - bounceOut(t4)), mix(0.5, 0.7, bounceOut(t1) - bounceOut(t3)));
    vec2 mPos = fPos - offset2;
    color = mix(color, COLOR_M, smoothstep(0.005, -0.005, sdRect(mPos, offset22)));

    vec2 offset3 = vec2(mix(-0.9, 0.8, bounceOut(t1) - bounceOut(t3)), mix(0.9, -0.8, bounceOut(t2) - bounceOut(t4)));
    vec2 offset33 = vec2(mix(0.5, 0.7, bounceOut(t1) - bounceOut(t3)), mix(0.5, 0.7, bounceOut(t2) - bounceOut(t4)));
    vec2 kPos = fPos - offset3;
    color = mix(color, COLOR_K, smoothstep(0.005, -0.005, sdRect(kPos, offset33)));

    vec2 offset4 = vec2(mix(0.9, -0.8, bounceOut(t2) - bounceOut(t4)), mix(0.9, -0.8, bounceOut(t1) - bounceOut(t3)));
    vec2 offset44 = vec2(mix(0.5, 0.7, bounceOut(t2) - bounceOut(t4)), mix(0.5, 0.7, bounceOut(t1) - bounceOut(t3)));
