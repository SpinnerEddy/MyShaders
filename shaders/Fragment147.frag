...
vec3 renderingFunc(vec2 uv){
    vec3 color = vec3(0.0);
	vec3 camPos = vec3(3.0 * cos(time), 1.0, 3.0 * sin(time));
	...
	if(hit){
		...
		float frenel = dot(normalize(rayDir), normal);
		vec3 dir = vec3(1.0, 1.0, 0.0);
		float d = dot(normalize(p), dir);
		color = mix(color, vec3(0.9137, 0.1216, 0.1216), pow(fract(d * 4.0 - time * 2.0), 6.0));
	}
